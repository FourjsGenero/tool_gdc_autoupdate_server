...
DEFINE serverSideArchive, serverSidePath, clientSideArchive, clientWindowsPath STRING
        ...
        LET serverArchive =  os.Path.dirNext(h) --switch to next
            WHILE TRUE
                ...
                IF os.Path.extension (serverArchive) == "zip" THEN
                    LET serverArchive = os.path.join(osServerSidePath,serverArchive)
                    EXIT WHILE
                ELSE 
                    LET serverArchive = os.Path.dirNext(h)
                        IF serverArchive IS NULL THEN 
                             LET msg = "There is no zip available in the corresponding OS directory"
                                CLOSE WINDOW SCREEN
...
CALL ui.Interface.frontCall ("standard", "feInfo", ["datadirectory"], [clientPath])
...
FUNCTION getGdcVersion ()
    ...
    CALL ui.Interface.frontCall ("standard", "feInfo", ["fePath"], [gdcInstallPath])
    ...
    ELSE
        LET msg = "The version available on the update server (", gdcServerArchive, ") is more recent than yours (", gdcVersion ,"). Do you want to update?"
